...
public class XzAssetReturnController extends BaseController {
    ...
    @Autowired
    private IXzPersonalApplyService personalApplyService;
    @Autowired
    ...
    @GetMapping("/xzPersonalApply/{code}")
    public String xzPersonalApply(@PathVariable("code") String code, ModelMap map) {

        XzAssetHandRecord xzAssetHandRecord = xzAssetHandRecordService.selectXzAssetHandRecordByAssetsCode(code);
        XzPersonalApply xzPersonalApply = new XzPersonalApply();
        if (xzAssetHandRecord.getApplyId() != null) {
            xzPersonalApply = personalApplyService.selectXzPersonalApplyById(xzAssetHandRecord.getApplyId());
            map.put("flg", "true");
        } else {
            map.put("flg", "false");
        }
        map.put("xzPersonalApply", xzPersonalApply);
    ...
    @PostMapping("/add")
    @ResponseBody
    public AjaxResult addSave(XzAssetReturn xzAssetReturn) {    //增加一条资产归还宝贝记录
