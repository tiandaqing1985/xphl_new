...
//				  				"审批提醒",user.getUserName()+"  "+
     ...
     * 财务审批通过提醒邮件
     **/
    public void facEmail1() {
        List<FacUserApproval> facUserApproval = facUserApprovalMapper.selectChenggong();
        int count = 0;//记录邮件发送数量
        for (FacUserApproval list : facUserApproval) {
            list.getApprovalState();
            EmailSend es = new EmailSend();
            SysUser user = userMapper.selectUserById(list.getApplicantId());
            String poname = "";
            if (list.getApplyId().substring(0, 2).equals("CL")) {
                poname = "差旅申请";
            } else if (list.getApplyId().substring(0, 2).equals("TJ")) {
                poname = "团建申请";
            } else if (list.getApplyId().substring(0, 2).equals("DG")) {
                poname = "对公申请";
            } else if (list.getApplyId().substring(0, 2).equals("JK")) {
                poname = "借款申请";
            } else if (list.getApplyId().substring(0, 2).equals("ZD")) {
                poname = "招待申请";
            } else if (list.getApplyId().substring(0, 2).equals("BX")) {
                poname = "日常报销申请";
            }
            try {
                if (list != null) {
     ...
     * 财务审批驳回提醒邮件
     **/
    public void facEmail2() {
        List<FacUserApproval> facUserApproval = facUserApprovalMapper.selectBoHui();
