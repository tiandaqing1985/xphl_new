...
public class XzPurchaseResourceApplyServiceImpl implements IXzPurchaseResourceApplyService {
    ...
    public List<XzPurchaseResourceApply> selectXzPurchaseResourceApplyList(XzPurchaseResourceApply xzPurchaseResourceApply) {
        List<XzPurchaseResourceApply> xzPurchaseResourceApplies = xzPurchaseResourceApplyMapper.selectXzPurchaseResourceApplyList(xzPurchaseResourceApply);
        for (XzPurchaseResourceApply apply : xzPurchaseResourceApplies) {
            apply.setApproverName(xzApplyProcessService.selectApplyProcessApproverNameByApplyId(apply.getId()));
        }

        return xzPurchaseResourceApplies;
    }

    @Override
    public List<XzPurchaseResourceApply> selectXzPurchaseResourceApplyListByApprover(XzPurchaseResourceApply xzPurchaseResourceApply) {

        List<XzPurchaseResourceApply> xzPurchaseResourceApplies = xzPurchaseResourceApplyMapper.selectXzPurchaseResourceApplyListByApprover(xzPurchaseResourceApply);
        for (XzPurchaseResourceApply apply : xzPurchaseResourceApplies) {
            apply.setApproverName(xzApplyProcessService.selectApplyProcessApproverNameByApplyId(apply.getId()));
        }
    ...
    public int insertXzPurchaseResourceApply(XzPurchaseResourceApply xzPurchaseResourceApply) {
        ...
        //审批状态：1同意 2驳回 3未审批 4审批中
        xzPurchaseResourceApply.setApprovalStatus("3");
        //提交状态：1保存 2提交
        int i = xzPurchaseResourceApplyMapper.insertXzPurchaseResourceApply(xzPurchaseResourceApply);

        Set<Long> xzPurchaseResourceProcess = xzApplyProcessService.createXzPurchaseResourceProcess(xzPurchaseResourceApply.getId(), xzPurchaseResourceApply.getApplyUserId(), xzPurchaseResourceApply.getApplyPrice());
        if (xzPurchaseResourceProcess.size() == 0) {
            XzPurchaseResourceApply updateVo = new XzPurchaseResourceApply();
            updateVo.setId(xzPurchaseResourceApply.getId());
            updateVo.setApprovalStatus("1");
            xzPurchaseResourceApplyMapper.updateXzPurchaseResourceApply(updateVo);
        }
